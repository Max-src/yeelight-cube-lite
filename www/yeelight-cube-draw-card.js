...
class YeelightCubeDrawCard extends LitElement {
  ...
  _renderPixelArtCarousel(
    ...
    const currentIndex = this._galleryCarouselIndex || 0;
    const validIndex = Math.max(
      0,
      Math.min(currentIndex, pixelArts.length - 1),
    );
    const art = pixelArts[validIndex];
      ...
      <div class="pixelart-gallery-carousel">
        <button
          ...
          0
            ...
            : ""}"
          title="Previous"
          @click=${() => this._navigateCarousel(-1, pixelArts.length)}
        ...
        <div class="carousel-content">
          ...
          <div class="carousel-indicators">
            ${Array.from(
              { length: pixelArts.length },
              (_, i) => html`
                <span
                  class="carousel-dot ${i === validIndex ? "active" : ""}"
                  @click=${() => this._setCarouselIndex(i)}
                ></span>
              `,
            )}
          </div>
        ...
        <button
          ...
          title="Next"
          @click=${() => this._navigateCarousel(1, pixelArts.length)}
          ?disabled=${validIndex === pixelArts.length - 1}
          style="     width: 38px !important;
                      max-width: 38px !important;
                      min-width: 38px !important;
                      height: 38px;
  ...
  _navigateCarousel(direction, maxLength) {
    ...
    const newIndex = Math.max(0, Math.min(current + direction, maxLength - 1));
...
window.customCards.push({
  ...
  name: "Yeelight Cube Draw Card",
  description: "Draw pixel art and control your Yeelight Cube matrix display.",
});
