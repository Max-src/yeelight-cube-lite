...
def _solar_flare_frames():
    parts = []
    index = 0
    while index < len(_SOLAR_FLARE_RLE):
        tier = _SOLAR_FLARE_RLE[index]
        count = int(_SOLAR_FLARE_RLE[index + 1 : index + 3], 36)
        parts.append(tier * count)
        index += 3
    flat = "".join(parts)
    return [flat[i : i + 100] for i in range(0, len(flat), 100)]


_SOLAR_FLARE_FRAMES = _solar_flare_frames()
_SOLAR_FLARE_TIER_HEAT = (0.0, 0.0, 0.48, 0.68, 1.0)
...
def _render_solar_flare(
    ...
    """Replay mode 19 directly from the quantised hardware recording.

    Every frame shown is one the lamp actually displayed: the preview steps
    through the recorded tier table at the recording's own 10 fps cadence.
    """
    steps = len(_SOLAR_FLARE_FRAMES)
    step = int(phase * 8.63) % steps  # 10 fps at the default speed setting
    frame = _SOLAR_FLARE_FRAMES[step]
    ...
    for row in range(ROWS):
        for col in range(COLS):
            if direction in ("Right", "Down"):
                ...
                lane = ROWS - 1 - row
                cell = col
            tier = frame[lane * 20 + cell]
            if tier == "0":
                texture = _value_noise_3d(cell * 0.31, lane * 0.47, phase * 0.18)
                heat = 0.10 + 0.06 * texture
            elif tier == "1":
                texture = _value_noise_3d(cell * 0.31, lane * 0.47, phase * 0.18)
                heat = 0.22 + 0.05 * texture
            else:
                heat = _SOLAR_FLARE_TIER_HEAT[int(tier)]
            pixels.append(_palette(_SOLAR_FLARE_PALETTE, heat))
